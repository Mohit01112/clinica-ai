...
# Empty = Pinecone default namespace
NAMESPACE = ""
...
def ensure_index(pc: Pinecone) -> None:
    ...
    for index_info in pc.list_indexes():
        if isinstance(index_info, dict):
            ...
            index_names.add(
                getattr(index_info, "name", None) or str(index_info)
            )
...
def delete_all_vectors(index) -> None:
    """
    Clears vectors from the current active-report session.

    Uses the default Pinecone namespace because NAMESPACE is empty.
    """
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not clear old report data from Pinecone: {error}"
        ) from error
...
def ingest_reports(uploaded_files) -> int:
    ...
    if not chunks:
        raise ValueError(
            "The uploaded PDF(s) did not produce usable text chunks."
        )
...
def retrieve(
    ...
    if NAMESPACE:
        query_arguments["namespace"] = NAMESPACE
...
def answer_as_clinica_ai(
    ...
    report_context = format_context(
        retrieved_documents
    )

    system_prompt = GP_SYSTEM_PROMPT.format(
    ...
    return extract_text_from_llm_content(
        response.content
    )
...
def render_upload_screen() -> None:
    ...
    st.title(
        "Clinica AI — Medical Report Assistant 🩺"
    )

    st.write(
        "Upload one or more medical-report PDFs, "
        "then ask questions about your results."
    )
    ...
    with st.spinner(
        ...
        try:
            ...
            st.session_state.error = (
                f"Could not process the report: {error}"
            )
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner(
            ...
            try:
                ...
                answer = (
                    f"I could not complete that request: "
                    f"{error}"
                )
